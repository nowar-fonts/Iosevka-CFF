...
const FINAL_SIMPLIFY_RESOLUTION = 4;
...
class FairizedShapeSink {
	...
	arcTo(arc, x, y) {
		const offPoints = CurveUtil.autoCubify(arc, FINAL_SIMPLIFY_TOLERANCE);
		for (let i = 0; i < offPoints.length; i++) {
			const z = offPoints[i];
			if (i % 3 == 2)
				this.lastContour.push(Point.cornerFrom(z).round(FINAL_SIMPLIFY_RESOLUTION));
			else
